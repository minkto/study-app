...
export const GET = withApiErrorHandling(async (currentUser: AppUser, _request: Request, { params }: { params: Promise<{ "chapter-id": string, "note-id": string }> }) => {

    const idSlugs = (await params);
    const chapterIdNum = Number(idSlugs["chapter-id"]);
    const noteIdNum = Number(idSlugs["note-id"]);

    const response = await getNote(noteIdNum, chapterIdNum, currentUser.userId);
    if (!response) {
...
export const PUT = withApiErrorHandling(async (currentUser: AppUser, request: Request, { params }: { params: Promise<{ "chapter-id": string, "note-id": string }> }) => {
